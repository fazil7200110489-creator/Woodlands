  ...
  return (
    <main className="min-h-screen bg-bg px-4 py-6 md:px-10">
        ...
        <div className="h-64">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={chartData}>
              ...
              <YAxis stroke="#8B7355" />
              <Bar dataKey="value" fill="#BF976A" radius={[6, 6, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </section>
      ...
      <section className="glass mb-6 rounded-3xl p-5">
        ...
        <div className="overflow-auto">
          <table className="w-full text-sm">
            <thead><tr className="text-left text-[#1D0F07]/60"><th>Order ID</th><th>Customer Name</th><th>Items</th><th>Amount</th><th>Pickup Time</th><th>Status</th></tr></thead>
            <tbody>
              {orders.map((o) => (
                <tr key={o._id} className="border-t border-[#1D0F07]/8">
      ...
      <section className="glass mb-6 rounded-3xl p-5">
        ...
        <div className="mb-4 grid gap-2 rounded-xl bg-[#1D0F07]/4 p-3 md:grid-cols-5">
          <input placeholder="Item name" className="rounded bg-[#1D0F07]/6 px-2 py-1" value={newItem.name} onChange={(e) => setNewItem({ ...newItem, name: e.target.value })} />
          <input placeholder="Price" type="number" className="rounded bg-[#1D0F07]/6 px-2 py-1" value={newItem.price} onChange={(e) => setNewItem({ ...newItem, price: Number(e.target.value) })} />
          <select className="rounded bg-[#1D0F07]/6 px-2 py-1" value={newItem.category} onChange={(e) => setNewItem({ ...newItem, category: e.target.value })}>{categories.map((c) => <option key={c}>{c}</option>)}</select>
          <input placeholder="Image URL" className="rounded bg-[#1D0F07]/6 px-2 py-1" value={newItem.image} onChange={(e) => setNewItem({ ...newItem, image: e.target.value })} />
        ...
        <div className="space-y-2">
          {menu.map((m) => (
            <div key={m._id || m.name} className="flex items-center gap-2 rounded-xl bg-[#1D0F07]/4 p-2">
              <input defaultValue={m.name} className="w-40 rounded bg-[#1D0F07]/6 px-2 py-1" onBlur={(e) => m._id && fetch("/api/menu", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...m, name: e.target.value }) }).then(load)} />
              <input defaultValue={m.price} type="number" className="w-24 rounded bg-[#1D0F07]/6 px-2 py-1" onBlur={(e) => m._id && fetch("/api/menu", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...m, price: Number(e.target.value) }) }).then(load)} />
              <select defaultValue={m.category} className="rounded bg-[#1D0F07]/6 px-2 py-1" onChange={(e) => m._id && fetch("/api/menu", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...m, category: e.target.value }) }).then(load)}>{categories.map((c) => <option key={c}>{c}</option>)}</select>
              <button className="rounded bg-[#1D0F07]/8 px-3 py-1" onClick={() => m._id && fetch("/api/menu", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...m, inStock: !m.inStock }) }).then(load)}>{m.inStock ? "In Stock" : "Out of Stock"}</button>
...
function Card({ label, value }: { label: string; value: string }) {
  return <div className="glass rounded-2xl p-4"><p className="text-sm text-[#1D0F07]/50">{label}</p><p className="mt-2 text-2xl font-semibold">{value}</p></div>;
}

function Toggle({ label, active, onClick }: { label: string; active: boolean; onClick: () => void }) {
  return <button onClick={onClick} className="flex items-center justify-between rounded-xl bg-[#1D0F07]/4 px-4 py-3"><span>{label}</span><span className={`rounded-full px-3 py-1 text-xs ${active ? "bg-emerald-500/30" : "bg-red-500/30"}`}>{active ? "ON" : "OFF"}</span></button>;
}
